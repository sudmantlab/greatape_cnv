...
def calc_vst(start_ind,end_ind,chrom,first_region,second_region,first_dict_cn, second_dict_cn,num_first,num_second,overall_grp,cn_grp):
    first_cns = [first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind] for samp in list(first_dict_cn.keys())]
    first_cns = np.mean(np.array(first_cns),axis=1)
    first_var = np.var(first_cns)
    if first_region != '' and second_region != '':
        second_cns = [second_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind] for samp in list(second_dict_cn.keys())]
...
def calc_variance(start_ind,end_ind,chrom,first_region,second_region,first_dict_cn, second_dict_cn,overall_grp,cn_grp):
    first_cns = [first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind] for samp in list(first_dict_cn.keys())]
    first_cns = np.mean(np.array(first_cns),axis=1)
    first_var = np.var(first_cns)
    if first_region != '' and second_region != '':
...
def calc_bp_density(start_ind,end_ind,chrom,first_region,second_region,first_dict_cn, second_dict_cn,example,overall_grp,cn_grp):
    if end_ind > len(first_dict_cn[example][overall_grp][chrom][cn_grp][...]):
        end_ind = len(first_dict_cn[example][overall_grp][chrom][cn_grp][...])
    mask_arr = [np.array(first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind]) for samp in list(first_dict_cn.keys())]
    mask_arr_shifted = [np.append(np.array([0]),np.array(first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind-1])) if start_ind == 0 else np.array(first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind-1:end_ind-1]) for samp in list(first_dict_cn.keys())]
    bp_density = np.mean([len(np.where((mask_arr[i]-mask_arr_shifted[i]) != 0)[0]) for i in np.arange(len(mask_arr))])
    return bp_density

def calc_pbs(start_ind,end_ind,chrom,first_region,second_region,third_region,first_dict_cn, second_dict_cn, third_dict_cn,num_first,num_second,num_third,overall_grp,cn_grp):
    first_cns = [first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind] for samp in list(first_dict_cn.keys())]
    first_cns = np.mean(np.array(first_cns),axis=1)
    first_var = np.var(first_cns)
    ...
    third_cns = [third_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind] for samp in list(third_dict_cn.keys())]
...
def calc_dcgh(start_ind,end_ind,chrom,first_region,dcgh_ind,zero_replacement,first_dict_cn,overall_grp,cn_grp):
    cns = [first_dict_cn[samp][overall_grp][chrom][cn_grp][start_ind:end_ind] for samp in list(first_dict_cn.keys()) if samp != dcgh_ind]
    cns = np.mean(np.array(cns), axis=1)
    dcgh_ind_cns = first_dict_cn[dcgh_ind][overall_grp][chrom][cn_grp][start_ind:end_ind]
...
def vst_variance_bpdensity_pbs_dcgh(args):
    ...
    for s in h5s:
        if (s[-3:] == '.h5' and s[:-suffix_len] in first_samp_list):
            ...
            first_dict_cn[s[:-suffix_len]] = h5py.File(samp_path,'r')
            """
            ...
            print(s[:-3])
            samp_path = os.path.join(h5_path,s)
            second_dict_cn[s[:-suffix_len]] = h5py.File(samp_path,'r')
            """
            samp_h5 = h5py.File(samp_path,'r')
            ...
            print(s[:-3])
            samp_path = os.path.join(h5_path,s)
            third_dict_cn[s[:-suffix_len]] = h5py.File(samp_path,'r')
            """
            samp_h5 = h5py.File(samp_path,'r')
    ...
    for human_chrom in list(h5_mask.keys()):
        t = time.time()
        vst_dict[human_chrom] = np.ones(len(h5_mask[human_chrom]['human_start'][...]))*-100
        matching_inds = np.where(h5_mask[human_chrom]['mask'][...] == 1)[0]
        print('CHROM: %s LEN CHROM: %s NUM MATCHING INDS: %s' % (human_chrom, len(h5_mask[human_chrom]['human_start'][...]),len(matching_inds)))
        chimp_chroms = [chimp_chrom_list[c] for c in h5_mask[human_chrom]['chimp_chrom'][...]]

        for ind in matching_inds:
            human_start = h5_mask[human_chrom]['human_start'][ind]
            human_end = h5_mask[human_chrom]['human_end'][ind]
            chimp_start = h5_mask[human_chrom]['chimp_start'][ind]
            chimp_end = h5_mask[human_chrom]['chimp_end'][ind]
            ...
            if max_max:
                ...
                for i in np.arange(len(start_chunks)):
                    if vst:
                        overall_list.append(calc_vst(int(start_chunks[i]),int(end_chunks[i]),chrom,first_region,second_region,first_dict_cn,second_dict_cn,num_first,num_second,overall_grp,cn_grp))
                    elif variance:
                        overall_list.append(calc_variance(int(start_chunks[i]),int(end_chunks[i]),chrom,first_region,second_region,first_dict_cn,second_dict_cn,overall_grp,cn_grp))
                    elif bp_density:
                        overall_list.append(calc_bp_density(int(start_chunks[i]),int(end_chunks[i]),chrom,first_region,second_region,first_dict_cn,second_dict_cn,example,overall_grp,cn_grp))
                    elif pbs:
                        overall_list.append(calc_pbs(int(start_chunks[i]),int(end_chunks[i]),chrom,first_region,second_region,third_region,first_dict_cn,second_dict_cn,third_dict_cn,num_first,num_second,num_third,overall_grp,cn_grp))
                    elif dcgh:
                        overall_list.append(calc_dcgh(int(start_chunks[i]),int(end_chunks[i]),chrom,first_region,dcgh_ind,zero_replacement,first_dict_cn,overall_grp,cn_grp))
                vst_dict[human_chrom][ind] = max(overall_list)
            else:
                if vst:
                    vst_dict[human_chrom][ind] = calc_vst(start_ind,end_ind,chrom,first_region,second_region,first_dict_cn,second_dict_cn,num_first,num_second,overall_grp,cn_grp)
                elif variance:
                    vst_dict[human_chrom][ind] = calc_variance(start_ind,end_ind,chrom,first_region,second_region,first_dict_cn,second_dict_cn,overall_grp,cn_grp)
                elif bp_density:
                    vst_dict[human_chrom][ind] = calc_bp_density(start_ind,end_ind,chrom,first_region,second_region,first_dict_cn,second_dict_cn,example,overall_grp,cn_grp)
                elif pbs:
                    print('PBS!')
                    vst_dict[human_chrom][ind] = calc_pbs(start_ind,end_ind,chrom,first_region,second_region,third_region,first_dict_cn,second_dict_cn,third_dict_cn,num_first,num_second,num_third,overall_grp,cn_grp)
                elif dcgh:
                    vst_dict[human_chrom][ind] = calc_dcgh(start_ind,end_ind,chrom,first_region,dcgh_ind,zero_replacement,first_dict_cn,overall_grp,cn_grp)
    ...
    for human_chrom in list(h5_mask.keys()):
        ...
        chrom_grp.create_dataset('human_start',data=h5_mask[human_chrom]['human_start'][...])
        chrom_grp.create_dataset('human_end',data=h5_mask[human_chrom]['human_end'][...])
        chrom_grp.create_dataset('chimp_start',data=h5_mask[human_chrom]['chimp_start'][...])
        chrom_grp.create_dataset('chimp_end',data=h5_mask[human_chrom]['chimp_end'][...])
        chrom_grp.create_dataset('chimp_chrom',data=h5_mask[human_chrom]['chimp_chrom'][...])
        chrom_grp.create_dataset(out_grp,data=vst_dict[human_chrom][...])

    for ind in first_dict_cn.keys():
        first_dict_cn[ind].close()

    for ind in second_dict_cn.keys():
        second_dict_cn[ind].close()

    for ind in third_dict_cn.keys():
        third_dict_cn[ind].close()
